...
async function main() {
  ...
  const cartaoAtualizadoLucas = await prisma.cartao.findUnique({
    where: { id: cartaoLucas.id },
  });

  const cartaoAtualizadoLucas2 = await prisma.cartao.findUnique({
    where: { id: cartaoLucas2.id },
  });

  const cartaoAtualizadoMaria = await prisma.cartao.findUnique({
    where: { id: cartaoMaria.id },
  });
  ...
  console.log(
    `- Cartão Lucas (BB): R$ ${cartaoAtualizadoLucas!.limiteRestante
    } de R$ 5000`
  );
  console.log(
    `- Cartão Lucas (Nubank): R$ ${cartaoAtualizadoLucas2!.limiteRestante
    } de R$ 4000`
  );
  console.log(
    `- Cartão Maria (Itaú): R$ ${cartaoAtualizadoMaria!.limiteRestante
    } de R$ 3000`
  );
}
